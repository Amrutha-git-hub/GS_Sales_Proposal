            ...
            if st.button(button_text, 
                ...
                try:
                    if is_selected:
                        ...
                        ClientDataManager.update_client_data(selected_business_priorities=updated_priorities)
...
"""
            ...
            set_global_message("rror loading page styles")

        logger.debug("Creating top section with client name and URLs")
        client_enterprise_name = render_first_section(logger= logger, client_data = client_data, is_locked= is_locked)
        render_second_section(logger= logger, client_data = client_data, is_locked= is_locked)
        render_third_section(logger= logger, client_data = client_data, is_locked= is_locked)
        spoc_name_provided , spoc_linkedin_profile = render_fourth_section(logger= logger, client_data = client_data, is_locked= is_locked)
        render_fifth_section(spoc_name_provided,spoc_linkedin_profile,logger= logger, client_data = client_data, is_locked= is_locked)
        render_sixth_section(logger= logger, client_data = client_data, is_locked= is_locked)
