...
def seller_tab():
    """Main seller tab function with comprehensive error handling and logging."""
    try:
        ...
        _render_enterprise_details_section()
...
def _initialize_session_state():
    ...
    try:
        ...
        session_vars = {
            ...
            'seller_enterprise_details_content': "",
            'seller_requirements_content': "",
            'seller_website_urls_list': [],
            'last_seller_company_name': "",
            'seller_uploaded_file_path': None,
            'seller_rfi_pain_points_items': {},
            'seller_document_analyzed': False,
            'seller_linkedin_profiles': {},
            'last_searched_seller_spoc': "",
            'seller_scraping_in_progress': False,
            'seller_pending_scrape_url': None,
            'processing_all_seller_documents': False,
            'seller_uploaded_files_paths': {},
            'seller_services_by_file': {},
            'seller_services_items': {}
        }
...
def _render_top_section():
    ...
    try:
        ...
        with col1:
            _render_seller_name_input()
        
        with col2:
            _render_website_url_section()
...
def _render_seller_name_input():
    ...
    try:
        ...
        with button_col:
            _handle_find_urls_button(seller_enterprise_name)
        
        # Common area for spinner/status - displayed under both input and button
        _display_url_search_status(seller_enterprise_name)
        
        # Clear URLs if company name is cleared
        _handle_company_name_changes(seller_enterprise_name)
...
def _handle_find_urls_button(seller_enterprise_name):
    ...
    try:
        ...
        # Initialize URL search state if not exists
        if 'url_search_in_progress' not in st.session_state:
            st.session_state['url_search_in_progress'] = False
        
        if st.button("🔍 Find Website",
                    disabled=find_urls_disabled or st.session_state.get('url_search_in_progress', False),
            ...
            st.session_state['url_search_in_progress'] = True
            st.session_state['url_search_company'] = seller_enterprise_name.strip()
            st.rerun()
...
def _handle_company_name_changes(seller_enterprise_name):
...
def _display_url_search_status(seller_enterprise_name):
    """Display URL search status in a common area under name input and button."""
    try:
        # Check if URL search is in progress
        if st.session_state.get('url_search_in_progress', False):
            search_company = st.session_state.get('url_search_company', seller_enterprise_name)
            ...
            with st.spinner(f"🔍 Finding websites for '{search_company}'..."):
                try:
                    ...
                    st.session_state['seller_website_urls_list'] = urls_list
                    st.session_state['last_seller_company_name'] = search_company
                    
                    # Clear search state
                    st.session_state['url_search_in_progress'] = False
                    st.session_state.pop('url_search_company', None)
        ...
        elif st.session_state.get('seller_website_urls_list') and st.session_state.get('last_seller_company_name'):
            url_count = len(st.session_state['seller_website_urls_list'])
            company_name = st.session_state['last_seller_company_name']
...
def _render_website_url_section():
    ...
    try:
        ...
        with url_col:
            seller_website_url = _render_url_dropdown()
        
        # Render action buttons
        with btn1_col:
            _render_visit_website_button(seller_website_url)
        with btn2_col:
            _render_refresh_urls_button(seller_website_url)
        with btn3_col:
            _render_scrape_website_button(seller_website_url)
        
        # Handle pending scraping operation
        _handle_pending_scraping()
...
def _render_url_dropdown():
    """Render URL dropdown selection."""
    try:
        seller_enterprise_name = st.session_state.get('seller_enterprise_name_input', '')
...
def _render_visit_website_button(seller_website_url):
...
def _render_refresh_urls_button(seller_website_url):
...
def _handle_refresh_urls():
    ...
    try:
        ...
        if seller_name_provided:
            logger.info(f"Refreshing URLs for: {seller_enterprise_name}")
            
            with st.spinner("Refreshing website URLs..."):
                try:
                    urls_list = get_urls_list(seller_enterprise_name)
                    st.session_state['seller_website_urls_list'] = urls_list
...
def _render_scrape_website_button(seller_website_url):
...
def _handle_pending_scraping():
    """Handle pending website scraping operation."""
    try:
        if (st.session_state.get('seller_scraping_in_progress') and 
            st.session_state.get('seller_pending_scrape_url')):
            
            scrape_url = st.session_state['seller_pending_scrape_url']
            ...
            with st.spinner(f"Scraping website details from {scrape_url}..."):
                try:
                    ...
                    st.session_state['last_analyzed_seller_url'] = scrape_url
...
def _render_document_upload_section():
    ...
    try:
        ...
        if seller_documents_upload and len(seller_documents_upload) > 0:
            _handle_uploaded_documents(seller_documents_upload)
...
def _handle_uploaded_documents(seller_documents_upload):
    ...
    try:
        ...
        _display_uploaded_files(seller_documents_upload)
        
        # Single button to process all files
        st.markdown("---")
        _render_process_all_button(seller_documents_upload)
        
        # Handle processing when button is clicked
        if st.session_state.get('processing_all_seller_documents', False):
            _process_all_documents(seller_documents_upload)
...
def _display_uploaded_files(seller_documents_upload):
...
def _render_process_all_button(seller_documents_upload):
    ...
    try:
        ...
        if analyze_all_clicked and not button_disabled:
            _handle_analyze_all_click()
            
    except Exception as e:
        logger.error(f"Error rendering process all button: {str(e)}", exc_info=True)

def _handle_analyze_all_click():
    """Handle analyze all documents button click."""
    try:
        seller_enterprise_name = st.session_state.get('seller_enterprise_name_input', '')
        ...
        if not seller_enterprise_name:
            ...
            logger.info("Starting processing of all seller documents")
            st.rerun()
            
    except Exception as e:
        logger.error(f"Error handling analyze all click: {str(e)}", exc_info=True)

def _process_all_documents(seller_documents_upload):
    ...
    try:
            ...
            with st.spinner(progress_text):
                try:
                    ...
                    if file_path and seller_enterprise_name:
                        ...
                        logger.error(f"Error saving file: {uploaded_file.name}")
        ...
        st.session_state['seller_document_analyzed'] = processed_count > 0
        st.session_state['processing_all_seller_documents'] = False
        
        # Show final summary
        _show_processing_summary(processed_count, total_files)
        
        logger.info(f"Document processing completed. {processed_count}/{total_files} files processed successfully")
        st.rerun()
        
    except Exception as e:
        logger.error(f"Error processing all documents: {str(e)}", exc_info=True)
        st.session_state['processing_all_seller_documents'] = False
        st.error("Error occurred while processing documents")

def _show_processing_summary(processed_count, total_files):
    ...
    try:
        ...
        logger.error(f"Error showing processing summary: {str(e)}", exc_info=True)

def _render_enterprise_details_section():
    ...
    try:
        ...
        seller_enterprise_details, seller_enterprise_details_provided = render_three_column_selector_unified(
            ...
            # Unified right section configuration
